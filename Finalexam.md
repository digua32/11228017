# 工程數學作業解答：拉普拉斯轉換與傅立葉級數

本文件提供拉普拉斯轉換作業第 (5) 題與傅立葉變換作業第 (7) 題的詳細求解步驟，格式已針對 GitHub Markdown 進行優化，方便複製與閱讀。
...
為了方便計算，我們先令基礎函數 $F(s) = \frac{1}{s(s+4)}$，並求其對應的時間域函數 $f(t) = \mathcal{L}^{-1}\{F(s)\}$。

#### Step 2: 使用部分分式展開法 (Partial Fraction Expansion) 求解 $f(t)$
將 $F(s)$ 拆解為部分分式：
$$\frac{1}{s(s+4)} = \frac{A}{s} + \frac{B}{s+4}$$

利用覆蓋法 (Heaviside cover-up method) 求係數 $A$ 與 $B$：
